...
import { Form, Select, Modal, Row, Col, Icon, Input, message, InputNumber, Checkbox } from 'antd';
import { connect } from 'react-redux';
const FormItem = Form.Item;
const createForm = Form.create;
const Option = Select.Option;
import ModalTableRow from './modalTableRow_v2';
...
let SendModal = React.createClass({
    ...
    updateDataSource(index) {
        return function (dataSource) {
            let wholeDataSource = Object.assign([], [...this.props.factorySendState.info]);
            wholeDataSource[index] = dataSource;
            store.dispatch(sendModalDataSource({info : wholeDataSource}));
        }.bind(this);
    },
    ...
    generateSum() {
        ...
        for (let i = 0; i < generatedSum.length; i++) {
            if (!goodsView[generatedSum[i]['goods_name']]) {
                ...
                goodsView[generatedSum[i]['goods_name']] = parseFloat(goodsView[generatedSum[i]['goods_name']]) +  parseFloat(generatedSum[i]['goods_num']);
            }
        };
        goodsView['总价'] = total;
